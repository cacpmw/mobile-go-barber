...
interface HourProperties {
  available: boolean;
  selected: boolean;
}

interface HourTextProperties {
  selected: boolean;
}
...
export const Hour = styled(RectButton)<HourProperties>`
  padding: 12px;
  background: ${props => (props.selected ? '#ff9000' : '#3e3b47')};
  border-radius: 10px;
  margin-right: 8px;
...
export const HourText = styled.Text<HourTextProperties>`
  color: ${props => (props.selected ? '#232129' : '#f4ede8')};
  font-family: 'RobotoSlab-Regular';
  font-size: 18px;
`;
